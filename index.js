...
function customAssign(...objects) {
  ...
  let mergObj = Object.assign({}, ...objects);

  for (const key in mergObj) {
    if (typeof mergObj[key] === "number") {
      mergObj[key] = String(mergObj[key]);
      mergObj[key] = mergObj[key].replace(/ /g, "_");
    }
  }
  return mergObj;
...
function filterObjectsByKey(arr, key) {
  ...
  let arrFilter = arr.filter(function (n) {
...
function getObjectValuesSum(obj) {
  ...
  let sum = values.reduce((ac, currentValue) => ac + currentValue, 0);
  return sum;
